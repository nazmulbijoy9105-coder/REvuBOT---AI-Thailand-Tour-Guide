...
async function callLLM(messages: Array<{ role: string; content: string }>) {
  ...
  // FIX 1: Removed `thinking: { type: 'disabled' }` — OpenAI rejects this field with a 400 error
...
export async function POST(req: NextRequest) {
  try {
    ...
    // FIX 2: Changed role from 'assistant' to 'system' — OpenAI requires system prompt as role: 'system'
    const llmMessages = [
